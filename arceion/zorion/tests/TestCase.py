...
class TestCase(TestCaseAbstract):
	...
	Description:
		Controller class is used to test API endpoints by sending requests to the API endpoints and receiving
		responses.
    ...
    def post(self, endpoint: str, data: dict, *, isAuthorized: bool, retry: bool = True):
        ...
        if not isAuthorized:
            return self.client.post(
                reverse(self.api + endpoint), data=data, follow=True, secure=True, content_type='application/json',
            )
        response = self.client.post(
