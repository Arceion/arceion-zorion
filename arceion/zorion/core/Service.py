...
from ..db.models import Model, QuerySet
...
class Service(serializers.ModelSerializer):
	...
	def __init__(self, *args, model: type[Model] | type[DjangoModel] | None = None, **kwargs):
	...
	def match(self, data: type[Request] | type[ModelRequest]) -> QuerySet:
		return self.search(data.validated_data)

	def search(self, data: dict) -> QuerySet:
